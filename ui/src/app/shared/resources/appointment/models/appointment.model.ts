...
export class Appointment {
  ...
  constructor(private appointment: any) {}

  get id(): string {
    return this.appointment?.uuid;
  }

  get personUuid(): string {
    return this.appointment?.person.uuid;
  }

  get identifier(): string {
    return (this.appointment?.identifiers || [])[0]?.display;
  }

  get MRN(): string {
    return (_.filter(this.appointment?.identifiers || []),
    (id) => {
  ...
  get name(): string {
    return this.appointment?.person?.display;
  }

  get fname(): string {
    return this.appointment?.person?.preferredName?.givenName;
  }

  get lname(): string {
    return this.appointment?.person?.preferredName?.familyName;
  }

  get dob(): string {
    return this.appointment?.person?.birthdate;
  }
  ...
  get gender(): string {
    return GenderType[this.appointment?.person?.gender];
  }
  ...
  get phoneNumber(): string {
    let phoneNumber = "";
    const attributes = this.appointment?.attributes || [];
  ...
  get cityVillage(): string {
    return this.appointment?.person?.preferredAddress?.cityVillage;
  }

  get council(): string {
    return this.appointment?.person?.preferredAddress?.address1;
  }

  get district(): string {
    return this.appointment?.person?.preferredAddress?.address2;
  }

  get region(): string {
    return this.appointment?.person?.preferredAddress?.address3;
  }
  get street(): string {
    return this.appointment?.person?.preferredAddress?.cityVillage;
  }
  ...
  getAttributeByTypeName(name): any {
    let attributeObject = _.filter(
      this.appointment?.person?.attributes,
      (attribute) => {
